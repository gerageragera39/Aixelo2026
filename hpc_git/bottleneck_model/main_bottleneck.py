...
from train_bottleneck import train_bottleneck_optimized
import argparse
import os
from utils import plot_histories
import random
from data_loader import load_dataset_folder, make_dataloaders_folder
...
if __name__ == "__main__":
    ...
    # seeds = [random.randint(0, 10**6) for _ in range(args.num_seeds)]
    seeds = [565473, 20917, 75025]

    print(f"Seeds: {seeds}")

    trainset, testset, num_classes = load_dataset_folder(data_dir=args.data_dir)
    trainloader, testloader = make_dataloaders_folder(trainset, testset, args.batch)
    ...
    for s in seeds:
        ...
        config_no_sd = BottleneckModelConfig(
            seed=s,
            depth=args.depth,
            sd_on=False,
        ...
        print("=" * 60)
        log_df, train_time, avg_batch_time = train_bottleneck_optimized(config_no_sd, trainloader, testloader, num_classes, learning_rate=args.lr)
        ...
        config_sd = BottleneckModelConfig(
            seed=s,
            depth=args.depth,
            sd_on=True,
            ...
            epochs=args.epochs,
            batch_size=args.batch,
            output_dir=os.path.join(args.out, f"{s}/with_sd_per_batch"),
        )

        print("\n" + "=" * 60)
        print("EXPERIMENT 2: WITH Stochastic Depth")
        print("=" * 60)
        log_df, train_time, avg_batch_time = train_bottleneck_optimized(config_sd, trainloader, testloader, num_classes, learning_rate=args.lr)
        log_dfs.append(log_df)
        training_times.append(train_time)
        avg_batch_times.append(avg_batch_time)
        ...
        print("=" * 60)
        plot_histories(log_dfs, labels, training_times, avg_batch_times, args.out + f"/{s}")
        ...
        # # Save result
        # results_file = os.path.join(args.out, f"{s}/timing_results.txt")
        # with open(results_file, 'w') as f:
        #     f.write("Stochastic Depth Timing Analysis Results\n")
        #     f.write("="*50 + "\n")
