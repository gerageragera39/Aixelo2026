...
def create_cifar_optimizer(model_parameters):
    return optim.SGD(model_parameters, lr=INITIAL_LEARNING_RATE,
                     momentum=MOMENTUM, nesterov=True, weight_decay=WEIGHT_DECAY)


def adjust_learning_rate(optimizer, epoch):
    lr = INITIAL_LEARNING_RATE
    if epoch >= LR_DECAY_EPOCHS[1]:
...
def plot_histories_cifar100(log_dfs, labels, times, avg_batch_times, output_dir):
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "cifar100_training_comparison.png")
