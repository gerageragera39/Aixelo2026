...
class BottleneckModelConfig:
    ...
    def __post_init__(self):
        # Valid ResNet depths for bottleneck architecture
...
class BottleneckBlock(nn.Module):
    """Bottleneck residual block for ResNet-50, ResNet-101, etc."""
    expansion = 4
...
class ResNetBottleneck(nn.Module):
    """
    ResNet implementation with bottleneck blocks for depths 50, 101, etc.
    """
    def __init__(self, config: BottleneckModelConfig, num_classes: int = 102):
        ...
        if config.depth == 50:
            self.layers_config = [3, 4, 6, 3]  # [stage1, stage2, stage3, stage4]
        elif config.depth == 101:
            self.layers_config = [3, 4, 23, 3]  # [stage1, stage2, stage3, stage4]
        else:
        ...
        # Calculate total number of blocks for stochastic depth decay
    ...
    def _make_stage(self, in_channels, out_channels, num_blocks, stride, block_idx, total_blocks):
        ...
        for i in range(num_blocks):
            ...
            if self.sd_on:
                current_block_idx = block_idx + i

                l = current_block_idx + 1
                survival_prob = 1 - (l / total_blocks) * (1 - self.final_survival_prob)
            ...
            # For the first block in the stage, use the calculated stride and downsample
            if i == 0:
                layers.append(BottleneckBlock(in_channels, out_channels, stride, survival_prob, downsample))
                in_channels = out_channels * BottleneckBlock.expansion  # Update in_channels after first block
            else:
