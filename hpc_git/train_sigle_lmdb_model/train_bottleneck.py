...
def train_bottleneck_resnet(config: BottleneckModelConfig, trainloader, testloader, num_classes, cp_path: str = None, new_epochs: int = None, learning_rate: float = 0.1):
    set_seed()
    ...
    if cp_path is not None and os.path.isfile(cp_path):
        ...
        start_epoch = config.epochs
        config.epochs = start_epoch + new_epochs
        
        print(f"Resuming training from epoch {start_epoch} to {config.epochs}")
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        # for batch_idx, (data, target) in enumerate(pbar):
            ...
            if batch_idx % 500 == 0:
                print(f'Epoch [{epoch+1}/{config.epochs}], Step [{batch_idx+1}/{len(trainloader)}], Loss: {loss.item():.4f}')
