...
parser.add_argument('--data-dir', default='/workspace/dataset', type=str,
                    help='path to data directory (default: /workspace/dataset)')
parser.add_argument('--results-dir', default='/workspace/results', type=str,
                    help='path to results directory (default: /workspace/results)')
...
parser.add_argument('--target', required=True, type=str,
                    help='name of the target column in id_prop.csv')
...
if not args.resume:
    args.resume = os.path.join(MODEL_DIR, "model_best.pth.tar")
...
def main():
    ...
    torch_data_path = os.path.join(args.data_options[0], 'pkl_data')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        avg_loss = train(train_loader, model, criterion, optimizer, epoch, normalizer)
    ...
    # test best model1
    best_checkpoint = torch.load(os.path.join(MODEL_DIR, 'model_best.pth.tar'))
    model.load_state_dict(best_checkpoint['state_dict'])
    print('---------Evaluate Best Model on Train Set---------------')
    validate(train_loader, model, criterion, normalizer, test=True,
             csv_name=os.path.join(args.results_dir, 'train_results.csv'))
    print('---------Evaluate Best Model on Val Set---------------')
    validate(val_loader, model, criterion, normalizer, test=True,
             csv_name=os.path.join(args.results_dir, 'val_results.csv'))
    print('---------Evaluate Best Model on Test Set---------------')
    validate(test_loader, model, criterion, normalizer, test=True,
             csv_name=os.path.join(args.results_dir, 'predictions.csv'))
    train_loss_path = os.path.join(args.results_dir, 'train_loss.csv')
    with open(train_loss_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'loss'])
        for epoch_idx, loss_val in enumerate(train_loss_per_epoch, start=1):
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer):
    ...
    return losses.avg.item() if isinstance(losses.avg, torch.Tensor) else losses.avg
