...
def test(args, model, loader, criterion, fabric: Fabric):
    ...
    for g, lg, fg, target, ids in loader:
        ...
        if args.out_dims > 1:
            ...
            indiv_errors.append([test_error.item()])

        # Get predictions and truths as lists
        reals = target.squeeze(dim=-1).tolist()
        preds = output.squeeze(dim=-1).tolist()
        ...
        if args.out_dims == 1:
            test_data[f'prediction {args.out_names[0]}'] = preds if isinstance(preds, list) else [preds]
            test_data[f'truth {args.out_names[0]}'] = reals if isinstance(reals, list) else [reals]
        else:
        ...
        for i in range(args.out_dims):
            test_data[f'error {args.out_names[i]}'] = indiv_errors[-1][i]
