...
def train(args, model, loader, optimizer, criterion, fabric):
    ...
    epoch_mae = torch.zeros(2).to(fabric.local_rank) 

    # progress_bar = tqdm(enumerate(loader), total=len(loader), disable=fabric.global_rank != 0)
    # for iteration, (g, lg, fg, target, _) in progress_bar:
    for iteration, (g, lg, fg, target, _) in enumerate(loader):
        ...
        with torch.no_grad():
            mae = F.l1_loss(output.detach(), target.detach())
            
            current_loss = loss.item() * args.n_cum
            
            epoch_loss[0] += current_loss
            epoch_mae[0] += mae.item()      
            epoch_loss[1] += 1 
            epoch_mae[1] += 1   

        if fabric.global_rank == 0:
            print(
                f"Batch {iteration + 1}/{len(loader)} | "
    ...
    fabric.all_reduce(epoch_mae, reduce_op='sum') 

    epoch_loss = epoch_loss[0] / epoch_loss[1]
    epoch_mae = epoch_mae[0] / epoch_mae[1]       

    fabric.print(f'Train Epoch Result | Loss: {epoch_loss:.4f} | MAE: {epoch_mae:.4f}')
...
def validate(args, model, loader, criterion, fabric):
    ...
    # for g, lg, fg, target, _ in tqdm(loader, desc="Validating", disable=fabric.global_rank != 0):
    for iteration, (g, lg, fg, target, _) in enumerate(loader):
        with torch.no_grad():
            output, _, _, _, _ = model(g, lg, fg)
            ...
            if args.out_dims > 1:
                ...
                for i, err in enumerate(individual_errors):
                    epoch_indiv_error[i][0] += err.item()
                    epoch_indiv_error[i][1] += 1
            else:
                epoch_indiv_error[0][0] += error.item()
                epoch_indiv_error[0][1] += 1
    ...
    final_error = epoch_error[0] / epoch_error[1]
    final_mae = epoch_mae[0] / epoch_mae[1]

    if args.out_dims > 1:
        for i, err in enumerate(epoch_indiv_error):
            err = fabric.all_reduce(err, reduce_op='sum')
            epoch_indiv_error[i] = err[0] / err[1]

    error_str = f'Validation | Loss: {final_error:.4f} | MAE: {final_mae:.4f}'

    if args.out_dims > 1:
        for i in range(args.out_dims):
            error_str += f' | {args.out_names[i]} Loss: {epoch_indiv_error[i]:.4f}'

    fabric.print(error_str)

    return final_error, epoch_indiv_error
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=10, help='number of epochs to train for (default: 100)')
    ...
    parser.add_argument('--target-column', type=str, required=True, help='name of the target column in id_prop.csv')
