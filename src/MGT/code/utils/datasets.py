...
class StructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, process: bool = False, random_seed: int = 123, transform=None):
        ...
        assert args.target_column is not None, 'target_column must be specified!'
        assert args.target_column in header, f'target_column {args.target_column} not found in header: {header}'
        self.target_idx = header.index(args.target_column)
    ...
    def __getitem__(self, idx):
        ...
        target = float(self.id_prop_data[idx][self.target_idx])
        return g, lg, fg, torch.tensor(target, dtype=torch.float32), cif_id
