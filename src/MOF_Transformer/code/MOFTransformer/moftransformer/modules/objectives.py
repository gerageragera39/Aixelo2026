...
def compute_regression(pl_module, batch, normalizer):
    ...
    batch : dict
        Input batch containing features and targets.
    normalizer : Normalizer
        Normalizer for encoding/decoding target values.
        
    Returns
    -------
    dict
        Dictionary containing loss, predictions, labels, and CIF IDs.
    """
    ...
    # match shape with squeezed logits
    ...
    loss_metric = getattr(pl_module, f"{phase}_regression_loss")(ret["regression_loss"])
    mae_metric = getattr(pl_module, f"{phase}_regression_mae")(
        mean_absolute_error(ret["regression_logits"], ret["regression_labels"])
    )
    
    # Update R² score metric (for both train and val)
    r2_metric = getattr(pl_module, f"{phase}_regression_r2")
    r2_metric.update(ret["regression_logits"], ret["regression_labels"])

    if pl_module.write_log:
        pl_module.log(f"regression/{phase}/loss", loss_metric, sync_dist=True)
        pl_module.log(f"regression/{phase}/mae", mae_metric, sync_dist=True)
